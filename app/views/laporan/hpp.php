<div class="space-y-6 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        ...
        <div class="flex items-center gap-3">
            <a href="<?= BASE_URL ?>laporan/hpp?dari=<?= htmlspecialchars($dari) ?>&sampai=<?= htmlspecialchars($sampai) ?>&export=csv"
                class="btn-secondary">
                ...
                <thead>
                    <tr class="bg-stone-50/50 dark:bg-stone-900/50">
                        ...
                        <th
                            ...
                            Menu</th>
                        <th
                            class="px-6 py-4 text-xs font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest border-b border-stone-100 dark:border-stone-700/50 text-right">
                            Total Biaya Bahan</th>
                        <th
                            ...
                            Hasil Baik</th>
                        <th
                            class="px-6 py-4 text-xs font-bold text-brand-yellow uppercase tracking-widest border-b border-stone-100 dark:border-stone-700/50 text-right">
                            HPP / Unit</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-stone-50 dark:divide-stone-700/30">
                    <?php foreach ($data as $row):
                        ...
                        <tr class="hover:bg-stone-50/50 dark:hover:bg-stone-900/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-stone-700 dark:text-stone-300 text-right font-medium">
                                Rp<?= number_format($row['total_biaya'] ?? 0, 0, ',', '.') ?></td>
                            <td class="px-6 py-4 text-center">
                                <span
                                    class="bg-stone-100 dark:bg-stone-700 text-stone-600 dark:text-stone-300 px-2 py-1 rounded-lg font-bold text-xs"><?= htmlspecialchars($row['jumlah_baik']) ?></span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span
                                    class="text-sm font-black text-brand-yellow">Rp<?= number_format($hpp, 0, ',', '.') ?></span>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (empty($data)): ?>
                        <tr>
                            <td colspan="5"
                                class="px-6 py-12 text-center text-stone-400 dark:text-stone-500 font-medium italic">Tidak
