...
<div class="bg-white rounded-lg shadow p-6 max-w-4xl">
    <form action="<?= BASE_URL ?>menu/edit/<?= $menu['id'] ?>" method="POST" id="menuForm">
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            <div>
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nama" type="text" name="nama" value="<?= htmlspecialchars($menu['nama']) ?>" required>
            </div>
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2" for="harga_jual">Harga Jual (Rp)</label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="harga_jual" type="number" name="harga_jual" value="<?= htmlspecialchars($menu['harga_jual']) ?>" required>
            </div>
        </div>
            ...
            <?php else: ?>
                ...
                <?php foreach($resep as $index => $r): ?>
                    <div class="flex items-center gap-4 mb-3 resep-row">
                        ...
                        <div class="w-32">
                            <label class="block text-gray-700 text-xs font-bold mb-1">Kuantitas</label>
                            <input type="number" step="0.01" name="qty[]" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?= htmlspecialchars($r['qty']) ?>" placeholder="Qty">
                        </div>
                        <div class="w-16 pt-5">
                            <button type="button" class="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded btn-remove-row">&times;</button>
                        </div>
